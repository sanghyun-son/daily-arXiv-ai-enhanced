...
def create_batch_requests(data: List[Dict], language: str) -> List[Dict]:
...
def submit_batch_job(data: List[Dict], language: str, data_file: str) -> str:
    """Submit batch job to OpenAI"""
    client = OpenAI()
    
    # Create batch requests
    batch_requests = create_batch_requests(data, language)
    ...
    print(f'Uploaded batch file with ID: {batch_input_file.id}', file=sys.stderr)
    ...
    batch_job = client.batches.create(
        ...
        metadata={
            ...
            "date": data_file.split('/')[-1].split('.')[0]  # Extract date from filename
        }
    )
...
def main():
    ...
    # Submit batch job
    batch_job_id = submit_batch_job(data, language, args.data)
